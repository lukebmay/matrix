...
1. Shared homepage copy constants (`ROLE_SPECS`, email, saying) for sizing + scene.
...
6. Saying: word-wrap to narrow `COLS − 2` or desktop `min(40, COLS − 2×pad)`.
7. `paintFromLogical`: toggle `m-link` from logical `href` (shared col-1 saying/email).
...
- [x] Roles + email + saying fit without OOB; build green
- [x] Saying col-1 no longer stuck at link brightness after tip leaves
